import React, { Components, useState } from 'react';
import Notifications from './Notifications';


function SponsorsOld() {

  return (
    <div>

      <div className="tab-content">
        <div className="latest-posts" >
          <h3 className="classic-title" id="notifications"><span>Supported By</span></h3>
          <img src="images/wceacm.png" width="300" height="50" alt="WCE ACM Student Chapter" />

        </div>
      </div>







      {/*
      <div className="col-md-4" id="news">
        <div className="latest-posts">


          <ul className="nav nav-tabs">
            <li className="active"><a href="#tab1" data-toggle="tab"><i className="icon-award-1"></i>Supported By</a></li>
            <li><a href="#tab2" data-toggle="tab"><i className="icon-beaker"></i>Publication By</a></li>
          </ul>
          <br />

          <div className="tab-content">
            <div className="tab-pane fade in active" id="tab1">
              {/*
              <img src="images/iet.png" width="300" height="50" alt="ACM India" />
              <br />
              <br />

              <img src="images/teqip3logo.png" width="300" height="50" alt="TEQIP-III" /><br /><br />
              <img src="images/FinIQ.png" width="300" height="50" alt="FinIQ" />

              
              <img src="images/wceacm.png" width="300" height="50" alt="WCE ACM Student Chapter" />
              <br /><br />
              
            </div>
            
            <div className="tab-pane fade" id="tab2">

              <h5><a href="http://www.iccet2020.in/publications.html" target="_blank"><img src="image/ICCET2020.jpg" alt="Springer" /></a></h5>
              <br /><br />
              <h5></h5>


              <h5><a href="http://www.ijcseonline.org/" target="_blank"><img src="image/ijcse_logo.jpg" alt="International Journal of Computer Sciences and Engineering" /></a></h5>
              <img src="images/iferp.png" alt="IFERP" style={{ width: '100px', alignSelf: 'center' }} />
              <br />
              <h5>1. Journal Name:- Materials Today: Proceedings <br />
                ISSN:- 2214-7853 <br />
                Article Processing Fee + Publication Charges:- 10000 INR
              </h5>
              <br /><br />
              <h5>
                2. Journal Name:- IOP Conference Series: Material Science & Engineering <br />
                ISSN:- 1757-899X <br />
                Article Processing Fee + Publication Charges:- 10000 INR
              </h5>
              <br /> <br />
              <h5>
                3. Journal Name:- Psychology and Education <br />
                ISSN:- 0033-3077 <br />
                Article Processing Fee + Publication Charges:- 10000 INR
              </h5>

              <br /><br />
            </div>
           

          </div>



        </div>
        <br/><br/>

       

      </div>
    </div>
    */}
    </div>
  );
}

export default SponsorsOld;
